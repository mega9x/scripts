...
const pageCount = parseInt(
  document.querySelector("#ViewPerPageArea").querySelectorAll("a")[4].innerText
);
const pageUri =
  "https://cli.linksynergy.com/cli/publisher/programs/advertisers.php";
const applyPostUri =
  "https://cli.linksynergy.com/cli/publisher/programs/apply_confirmation.php";

const token = csrfMagicToken;
const defaultX = 61;
const defaultY = 9;
const priceRegex =
  /(USD|EUR|€|\$|£)\s?(\d{1,}(?:[.,]*\d{3})*(?:[.,]*\d*))|(\d{1,3}(?:[.,]*\d*)*(?:[.,]*\d*)?)\s?(USD|EUR)/g;
...
const run = async () => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (i != 1) {
      ...
      pageCurrent =
        parseInt(page.querySelector("input[name=currec]").value) + 100;
    }
    const trs = page
      .querySelector(".programTable > tbody")
      .querySelectorAll("tr");
    for (let t of trs) {
      let ifCon = false;
      ...
      for (let p of prices) {
        try {
          if (
            p[2].replace(dollarSignRegex, "") > maxMoney ||
            p[2].replace(dollarSignRegex, "") < minMoney
          ) {
            ifCon = true;
            console.log(p[2]);
            console.log("金额不匹配");
